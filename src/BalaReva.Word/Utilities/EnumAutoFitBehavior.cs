namespace BalaReva.Word.Utilities;

/// <summary>
/// Mirrors the <c>BalaReva.Word.Utilities.EnumAutoFitBehavior</c> of the published BalaReva.Word.Activities.
/// </summary>
/// <remarks>
/// Names and values are generated from the published assembly's metadata by
/// tools/generate_enums.py. A workflow persists the member name, and the
/// underlying number reaches .xaml through some expression forms, so neither
/// may be renamed or renumbered.
/// </remarks>
public enum EnumAutoFitBehavior
{
    /// <summary>Fit fixed column.</summary>
    FitFixedColumn = 0,

    /// <summary>Fit content.</summary>
    FitContent = 1,

    /// <summary>Fit window.</summary>
    FitWindow = 2,
}
